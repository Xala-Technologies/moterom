...
The installed Digdir CSS/React package version is recorded in `package-lock.json`. Brand typography is Digdir/Digilist Inter (self-hosted `@fontsource/inter`, weights 400/500/600) with Digdir `--ds-heading-*` / `--ds-body-*` / `--ds-font-size-*` tokens. Customer Dashboard and administrative pages share the same left sidebar pattern; Finn rom and guest pages keep the brand header and constrained content area. Brand files are excluded from automatic formatting to preserve their source content.

Admin screens follow Digilist’s tenant dashboard as a visual specification. That UI was inspected at Digilist `e8226e0d4db2fc6a3f71c168f62f0a61065ce4cf` (`feat/tenant-portal-listings`). Do not copy dashboard React, SDK hooks, or app-shell into this public repository. See [`shared-digilist-dashboard.md`](shared-digilist-dashboard.md).
