...
| Personal bookings, cancellation and calendar download                     | Existing paths retained; access-token cancellation contract preserved; retry recovery repaired                | Live end-to-end acceptance                                                        |
...
| Staff role changes and invitations                                        | Not called from Møterom; `inviteMember` remains Digilist-only until a booker role and magic-link return exist | Do not map access requests to staff invites                                       |
